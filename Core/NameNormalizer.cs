...
public sealed class NameNormalizer
{
    ...
    public string NormalizePersonName(string? value) => NormalizeCore(value, personName: true);
    ...
    public double Similarity(string? left, string? right)
    {
        ...
        // SequenceMatcher's default autojunk excludes frequent anchors at length >= 200.
    ...
    private string NormalizeCore(string? value, bool personName = false)
    {
        ...
        // These letters do not decompose into ASCII under NFKD. Keep this
        // mapping specific to players: club keys and aliases retain their behavior.
        if (personName)
        {
            cleaned = cleaned.Replace("æ", "ae").Replace('ø', 'o').Replace('ł', 'l');
        }
        ...
        var asciiish = builder.ToString().Normalize(NormalizationForm.FormC).ToLowerInvariant();
        asciiish = NonAlphaNum.Replace(asciiish, " ");
        return MultiSpace.Replace(asciiish, " ").Trim();
    }

    private static int CountMatchingBlocks(string a, int aStart, int aEnd, string b, int bStart, int bEnd, HashSet<char> popular)
    {
        ...
        return length
               + CountMatchingBlocks(a, aStart, ai, b, bStart, bj, popular)
               + CountMatchingBlocks(a, ai + length, aEnd, b, bj + length, bEnd, popular);
    }

    private static (int IndexA, int IndexB, int Length) LongestCommonSubstring(
        string a, int aStart, int aEnd, string b, int bStart, int bEnd, HashSet<char> popular)
    {
        ...
        // Popular characters may extend a match, but must not anchor one.
        ...
        while (bestA + best < aEnd && bestB + best < bEnd && a[bestA + best] == b[bestB + best])
        {
            best++;
        }
